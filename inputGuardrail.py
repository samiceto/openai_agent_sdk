...
class match_homework_output(BaseModel):
    reasoning:str
    is_math_homework:bool
...
guardrail_agent=Agent(
    name="Guardrail check",
    instructions="Check if the user is asking you to do their math homework.",
    output_type=match_homework_output
)
...
async def math_guardrail(
    ...
    return GuardrailFunctionOutput(
        output_info=result.final_output_as(match_homework_output),
...
async def main():
    agent = Agent(
        name="cutomer care",
        instructions="you are customer care agent",
        input_guardrails=[math_guardrail],
    )

    input_updat:list[TResponseInputItem] = []
    while True:
        ...
        try:
            result =await Runner.run (agent,input_updat,run_config=config)
            print(result.final_output)
        except InputGuardrailTripwireTriggered:
            message = "Sorry, I can't help you with your math homework."
            print(message)
